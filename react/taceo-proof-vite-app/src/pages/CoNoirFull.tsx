...
import { CoNoir, type ConfigurationParameters, JobApi, NodeApi, Configuration } from '@taceo/proof-client-bundler'
...
export default function CoCircomFull() {
  ...
  const abiFileRef = useRef<HTMLInputElement>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const abi = JSON.parse(await abiFile!.text());
      const input = JSON.parse(await file!.text());
      const jobId = await CoNoir.scheduleFullJob(jobInstance, nodes, blueprint, voucher, abi, new Uint32Array(publicInputs), input);
      ...
      <form className="w-full" onSubmit={handleSubmit}>
        <div className="grid gap-2">
          ...
          <div>
            ...
            <input required type="file" ref={abiFileRef} onChange={handleAbiFileChange} style={{ display: 'none' }} />
            <button className="rounded-[5pt] shadow-xl border border-current p-2 w-full cursor-pointer" onClick={handleAbiUploadClick} type="button">
              {abiFile ? abiFile .name : 'Choose File'}
            </button>
          </div>
          <div>
