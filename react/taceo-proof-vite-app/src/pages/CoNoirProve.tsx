...
import { CoNoir, type ConfigurationParameters, JobApi, NodeApi, Configuration, MpcProtocol } from '@taceo/proof-client-bundler'

export default function CoCircomProve() {
  ...
  const [mpcProtocol, setMpcProtocol] = useState<MpcProtocol>(MpcProtocol.Rep3);
  const [proof, setProof] = useState<Uint8Array | null>(null);
  const [publicInputsOut, setPublicInputsOut] = useState<Uint8Array | null>(null);
  const [publicInputs, setPublicInputs] = useState<Array<number>>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const witness = new Uint8Array(await file!.arrayBuffer());
      const jobId = await CoNoir.scheduleProveJob(jobInstance, nodes, blueprint, mpcProtocol, voucher, new Uint32Array(publicInputs), witness);
    ...
    <div className="flex items-center justify-center py-10 px-15 md:rounded-[10pt] md:shadow-xl md:border md:border-current md:w-lg">
      <form className="w-full" onSubmit={handleSubmit}>
        <div className="grid gap-2">
          ...
          <div>
            <h2 className="text-[14pt] font-bold pb-1">MPC Protocol</h2>
            <select required className="rounded-[5pt] shadow-xl border border-current p-2 w-full" onChange={(e) => setMpcProtocol(e.target.value as MpcProtocol)}>
          ...
          <div>
            <h2 className="text-[14pt] font-bold pb-1">Public Inputs</h2>
              <input className="rounded-[5pt] shadow-xl border border-current p-2 w-full" type="text" onChange={(e) => setPublicInputs(e.target.value.split(',').map((e) => parseInt(e)))} />
          </div>
          <div>
            <h2 className="text-[14pt] font-bold pb-1">Witness</h2>
          ...
          <div className="pt-5 mx-auto text-center">
            ...
            {proof && (
              <div>
                <a className="underline text-current" href={URL.createObjectURL(new Blob([proof], { type: 'application/octet-stream' }))} download="proof">
                  Download Proof
                </a>
                <br />
                <a className="underline text-current" href={URL.createObjectURL(new Blob([publicInputsOut!], { type: 'application/octet-stream' }))} download="public_inputs">
