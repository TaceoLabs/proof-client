...
import { CoCircom, type ConfigurationParameters, JobApi, NodeApi, Configuration, BlueprintCurve } from '@taceo/proof-client-bundler'

export default function CoCircomAddJobInputs() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const nodes = await Promise.all(jobNodes.map(async id => await nodeInstance.nodeProvider({ id })));
      const input = JSON.parse(await file!.text());
      CoCircom.addJobInputs(jobInstance, { node0: nodes[0], node1: nodes[1], node2: nodes[2] }, jobId!, curve, publicInputs, input);
      setLoading(false);
    } catch (error: any) {
    ...
    <div className="flex items-center justify-center py-10 px-15 md:rounded-[10pt] md:shadow-xl md:border md:border-current md:w-lg">
      <form className="w-full" onSubmit={handleSubmit}>
        <div className="grid gap-2">
          ...
          <div>
            <h2 className="text-[14pt] font-bold pb-1">Job ID</h2>
          ...
          <div>
            <h2 className="text-[14pt] font-bold pb-1">Job Nodes</h2>
              <input required className="rounded-[5pt] shadow-xl border border-current p-2 w-full" type="text" onChange={(e) => setJobNodes(e.target.value.split(',').map((e) => parseInt(e)))} />
          </div>
          <div>
