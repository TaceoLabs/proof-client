generate-js-api-client:
  (rm -r js/taceo-proof-api-client || true) && docker run --rm \
    ...
    --additional-properties=supportsES6=true,npmName=@taceo/proof-api-client,npmVersion=0.0.0 && cd js/taceo-proof-api-client && npm i
...
generate-rust-api-client:
  ...
  sed -i 's|// TODO: support file upload for '\''input_party0'\'' parameter|multipart_form = multipart_form.part("input_party0", reqwest::multipart::Part::stream(p_input_party0));|' src/apis/job_api.rs && \
  sed -i 's|// TODO: support file upload for '\''input_party1'\'' parameter|multipart_form = multipart_form.part("input_party1", reqwest::multipart::Part::stream(p_input_party1));|' src/apis/job_api.rs && \
  sed -i 's|// TODO: support file upload for '\''input_party2'\'' parameter|multipart_form = multipart_form.part("input_party2", reqwest::multipart::Part::stream(p_input_party2));|' src/apis/job_api.rs && \
